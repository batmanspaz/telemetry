...
const KEYISH = /\b(?:sk|pk|rk|ak|ghp|gho|ghs|ghu|github_pat|xox[baprs]|glpat)[-_][A-Za-z0-9/+_-]{8,}/i;
// AWS access key ids.
 ...
 */
export function scanForPii(event) {
    const leaks = [];
    for (const [key, val] of Object.entries(event.props ?? {})) {
        if (looksLikePii(val))
            leaks.push(`props.${key}`);
    }
    for (const field of ['actor', 'entity_id', 'session_id']) {
        if (looksLikePii(event[field]))
            leaks.push(field);
    }
    return leaks;
}
